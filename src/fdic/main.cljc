(ns fdic.main
  "Kotodama WASM entrypoint for the Fdic clean-room actor (L4) — Clojure port.
  ...
  facts (`fdic.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"fdic.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fdic-compat" :tier tier :entities entities} 200])
